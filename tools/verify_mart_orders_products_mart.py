...
if __name__ == '__main__':
    ...
    # --- Verification for marts.orders_products_mart ---
    print("\n--- Verifying marts.orders_products_mart ---")
    ...
    # 2. Extract source data (stg.stg_order JOIN stg.stg_order_items JOIN stg.stg_product) for these order_ids
    source_data_query = f"""
    SELECT
        o.order_id, o.customer_id, o.order_date, o.total_amount,
        oi.product_id, p.product_name, p.product_category,
        oi.quantity, oi.price AS item_price, p.price AS product_price, p.created_date AS product_created_date
    FROM stg.stg_order o
    JOIN stg.stg_order_items oi ON o.order_id = oi.order_id
    JOIN stg.stg_product p ON oi.product_id = p.product_id
    ...
    # 3. Extract target data (marts.orders_products_mart) for these order_ids
    target_data_query = f"""
    SELECT
        order_id, customer_id, order_date, total_amount,
        product_id, product_name, product_category,
        quantity, item_price, product_price, product_created_date
    FROM marts.orders_products_mart
    ...
    # 4. Print comparison
    print_data_comparison(source_data, target_data, source_name="stg.stg_order JOIN stg.stg_order_items JOIN stg.stg_product", target_name="marts.orders_products_mart")
