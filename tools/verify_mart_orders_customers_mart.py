...
if __name__ == '__main__':
    ...
    # --- Verification for marts.orders_customers_mart ---
    print("\n--- Verifying marts.orders_customers_mart ---")
    ...
    # 2. Extract source data (stg.stg_order JOIN stg.stg_customer) for these order_ids
    source_data_query = f"""
    SELECT
        o.order_id, o.customer_id, o.order_date, o.total_amount,
        c.customer_name, c.customer_email, c.registration_date
    FROM stg.stg_order o
    JOIN stg.stg_customer c ON o.customer_id = c.customer_id
    ...
    # 3. Extract target data (marts.orders_customers_mart) for these order_ids
    target_data_query = f"""
    SELECT
        order_id, customer_id, customer_name, customer_email, order_date, total_amount, registration_date
    FROM marts.orders_customers_mart
    ...
    # 4. Print comparison
    print_data_comparison(source_data, target_data, source_name="stg.stg_order JOIN stg.stg_customer", target_name="marts.orders_customers_mart")
